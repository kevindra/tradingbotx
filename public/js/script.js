...
$(document).ready(function () {
  ...
  $(document).on('click', '.add', function () {
    ...
    var tickerType = row.find('td.ticker-type').find('select option:selected').val();
...
$(document).on('click', '#save', function () {
  ...
  $('table tbody tr').each(function () {
    var record = {
      ...
      tickerType: $(this).find('td.ticker-type').find('select option:selected').val(),
...
$(document).on('click', '#refresh', function() {
    $('table tbody tr').each(function () {
        ...
        getTickerConf(record.ticker, record.tickerType, record.horizon, function (data) {
            row.find('td.price').html('$' + data[1]);
            row.find('td.buyconf').html(parseFloat(data[2]).toFixed(2) + '%');
            row.find('td.price-timestamp').html(new Date(data[0]).toUTCString());
        });
